...
public:
    ...
    explicit TemplateClientService(rclcpp::Node::SharedPtr node) 
    : node_(node), client_(nullptr) {}
    ...
    bool Connect() override {
        while(!client_->wait_for_service(2s))
        {
            ...
            RCLCPP_INFO(node_->get_logger(),"服务连接中！");
        }
    ...
    bool SendRequest(const void* request, void* output) {
        // 参数类型检查
        if (!request || !output) {
            RCLCPP_ERROR(node_->get_logger(), "缺少请求与输出！");
            return false;
        }
        // 类型转换
        ...
        // 阻塞等待响应
        auto result = rclcpp::spin_until_future_complete(node_->get_node_base_interface(),future);
        // 处理结果
        if (result == rclcpp::FutureReturnCode::SUCCESS) {
            *concrete_output = *future.get();
            return true;
        }
        RCLCPP_ERROR(node_->get_logger(), "服务端返回失败 (错误码: %d)", static_cast<int>(result));
        return false;
    }
