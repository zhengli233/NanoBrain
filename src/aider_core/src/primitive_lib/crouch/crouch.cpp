...
CrouchPrimitive::CrouchPrimitive(std::shared_ptr<rclcpp::Node> node)
    :node_(node){
    RCLCPP_INFO(node_->get_logger(), "动作基元：下蹲已激活！");//在CrouchPrimitive动作基元库中传递大脑节点
    //初始化参数列表
    REGISTER_PARAM("crouch_height",double, crouch_height);
    //在CrouchPrimitive动作基元库解析函数中实例化需要用到的服务通信
    crouch_to_client_ = std::make_shared<TemplateClientService<Crouch>>(node_);
...
bool CrouchPrimitive::Excute() {
    ...
    if(crouch_to_client_->SendRequest(&crouch_request,&crouch_response)==false) {
        RCLCPP_ERROR(node_->get_logger(),"未连接至下肢服务器，连接失败，程序退出！");
        return 0;
    } 
    //判断返回的数据是否识别成功
    if(crouch_response.success == 0) {
        RCLCPP_ERROR(node_->get_logger(),"下蹲失败！");
        return 0;
    }
    RCLCPP_INFO(node_->get_logger(), "下蹲成功！");
    return 1;
