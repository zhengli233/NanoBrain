...
SingleArmMoveToPrimitive::SingleArmMoveToPrimitive(std::shared_ptr<rclcpp::Node> node)
    :node_(node){
    RCLCPP_INFO(node_->get_logger(), "动作基元：单臂运动指定坐标点已激活！");//在SingleArmMoveToPrimitive动作基元库中传递大脑节点
    //初始化参数列表
